...
export default function TransactionTable({transactions}){
 ...
 useEffect(()=>{
    if(deleted && !deleteLoading) {
      setSelectedIds([]);
      toast.error("Transactions deleted sucessfully");
    }
 ...
 const handleClearFilters=()=>{
  ...
  {selectedIds.length>0 && <div>
    ...
    >
      ...
      {(searchTerm || typeFilter || recurringFilter) && (
        <Button variant="outline" size="icon" onClick={handleClearFilters} className="bg-blue-600">
          <X className="h-4 w-5  " />
        </Button>
      )}
    </div>
  ...
  <TableHeader>
    <TableRow>
      ...
      <div className="flex items-center">Date{sortConfig.field==='date'&&(
        sortConfig.direction==="asc"?<ChevronUp className="m1-1 h-4 w-4" />: <ChevronDown
        className="m1=1 h-4 w-4" />
      )}</div>
      </TableHead>
  ...
  <TableBody >
    ...
    <TableRow key={transaction.id}>
      ...
      checked={selectedIds.includes(transaction.id)} className="border-gray-600"
      />
      </TableCell>
      <TableCell>{formatDate(new Date(transaction.date),"PP")}</TableCell>
